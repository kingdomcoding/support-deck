defmodule SupportDeck.Tickets.Ticket do
  ...
  state_machine do
    ...
    transitions do
      ...
      transition :close,          from: [:new, :triaging, :assigned, :waiting_on_customer, :escalated, :resolved], to: :closed
    end
  end
  ...
  attributes do
    ...
    attribute :product_area, :atom do
      constraints one_of: [:auth, :database, :storage, :functions, :realtime, :dashboard, :billing, :general]
      public? true
    end
  ...
  actions do
    ...
    create :open do
      accept [
        :external_id, :source, :subject, :body, :severity, :product_area,
        :subscription_tier, :customer_email, :customer_id,
        :front_conversation_id, :slack_thread_ts, :slack_channel_id,
        :metadata
      ]
      ...
      change after_action(fn changeset, ticket, _ctx ->
    ...
    create :upsert do
      ...
      upsert_fields [
        :subject, :body, :severity, :product_area, :subscription_tier,
    ...
    update :begin_triage do
      ...
      change after_action(fn _changeset, ticket, _ctx ->
  ...
  calculations do
    calculate :sla_remaining_minutes, :integer, expr(
      fragment("EXTRACT(EPOCH FROM (? - NOW())) / 60", sla_deadline)
    )
  end
