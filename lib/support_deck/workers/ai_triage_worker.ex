defmodule SupportDeck.Workers.AITriageWorker do
  ...
  def perform(%Oban.Job{args: %{"ticket_id" => ticket_id}}) do
    ...
    {:ok, ticket} = Tickets.get_ticket(ticket_id)

    Logger.info("ai.triage.started", ticket_id: ticket_id)

    case classify(ticket) do
      {:ok, classification} ->
        ...
        Tickets.apply_ai_results(ticket, %{
          ...
          ai_confidence: classification["confidence"] || classification[:confidence],
          product_area: parse_area(classification),
          severity: parse_severity(classification)
        })

        AI.record_triage(ticket_id, %{
          predicted_category: classification["product_area"] || classification[:product_area],
          predicted_severity: classification["severity"] || classification[:severity],
          confidence: classification["confidence"] || classification[:confidence],
          is_repetitive: classification["is_repetitive"] || classification[:is_repetitive] || false,
          draft_response: draft,
          processing_time_ms: elapsed
        })

        is_rep = classification["is_repetitive"] || classification[:is_repetitive]
        conf = classification["confidence"] || classification[:confidence] || 0.0

        if is_rep && conf > 0.85 do
      ...
      {:error, reason} ->
        Logger.error("ai.triage.failed", ticket_id: ticket_id, reason: inspect(reason))
        {:error, reason}
    end
  end
  ...
  defp maybe_generate_draft(ticket, classification) do
    ...
    if is_rep && conf > 0.7 do
      area = classification["product_area"] || classification[:product_area]
  ...
  defp parse_area(%{product_area: area}), do: area
  ...
  defp parse_severity(%{severity: s}), do: s
