defmodule SupportDeckWeb.SimulatorLive do
  ...
  def mount(_params, _session, socket) do
    tickets = case SupportDeck.Tickets.list_open_tickets() do
      {:ok, t} -> t
      _ -> []
    end
  ...
  def handle_event("send_webhook", params, socket) do
    ...
    case Jason.decode(payload) do
      {:ok, _} ->
        {:noreply, assign(socket, :result, {:ok, "Webhook payload for #{source} would be sent to /webhooks/#{source}. Use curl or an HTTP client to POST the payload."})}
  ...
  def handle_event("trigger_triage", %{"ticket_id" => ticket_id}, socket) do
    case Oban.insert(SupportDeck.Workers.AITriageWorker.new(%{ticket_id: ticket_id})) do
      {:ok, _} ->
        {:noreply, assign(socket, :result, {:ok, "AI triage job queued for ticket #{String.slice(ticket_id, 0..7)}..."})}
  ...
  def handle_event("check_sla", _, socket) do
    case SupportDeck.Tickets.list_breaching_sla() do
      {:ok, tickets} ->
        {:noreply, assign(socket, :result, {:ok, "SLA check complete: #{length(tickets)} ticket(s) breaching"})}
  ...
  def handle_event("trip_breaker", %{"integration" => integration}, socket) do
    ...
    {:noreply, assign(socket, :result, {:ok, "#{integration} breaker state: #{status.state} (failures: #{status.failures})"})}
  end
  ...
  def render(assigns) do
    ...
    <div class="max-w-6xl mx-auto px-6 py-6">
      ...
      <div :if={@result} class={"mb-6 p-4 rounded-lg border #{if elem(@result, 0) == :ok, do: "bg-green-50 border-green-200", else: "bg-red-50 border-red-200"}"}>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div class="bg-white rounded-lg border border-gray-200 p-6">
          ...
          <form phx-submit="create_ticket" class="space-y-3">
            <div>
              ...
              <input type="text" name="subject" value={@ticket_form["subject"]} required class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" />
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Body</label>
              <textarea name="body" rows="3" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">{@ticket_form["body"]}</textarea>
            </div>
            <div class="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <select name="severity" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">
                  <option :for={s <- [:low, :medium, :high, :critical]} value={s} selected={to_string(s) == @ticket_form["severity"]}>{s}</option>
                </select>
              </div>
            </div>
            <div class="grid grid-cols-2 gap-3">
              <div>
                ...
                <select name="subscription_tier" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">
                  <option :for={t <- [:free, :pro, :team, :enterprise]} value={t} selected={to_string(t) == @ticket_form["subscription_tier"]}>{t}</option>
                </select>
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Customer Email</label>
                <input type="email" name="customer_email" value={@ticket_form["customer_email"]} class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" />
              </div>
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">External ID</label>
              <input type="text" name="external_id" value={@ticket_form["external_id"]} required class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" />
            </div>
            <button type="submit" class="w-full px-4 py-2 text-sm bg-indigo-600 text-white rounded-lg hover:bg-indigo-700">Create Ticket</button>
          </form>
        </div>
        ...
        <div class="space-y-6">
          <div class="bg-white rounded-lg border border-gray-200 p-6">
            ...
            <form phx-submit="send_webhook" class="space-y-3">
              ...
              <div>
                ...
                <textarea name="payload" rows="4" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm font-mono">{@webhook_form["payload"]}</textarea>
              </div>
              <button type="submit" class="w-full px-4 py-2 text-sm bg-indigo-600 text-white rounded-lg hover:bg-indigo-700">Send Webhook</button>
            </form>
          </div>
          ...
          <div class="bg-white rounded-lg border border-gray-200 p-6">
            ...
            <form phx-submit="trigger_triage" class="space-y-3">
              <div>
                ...
                <select name="ticket_id" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">
                  <option value="">Choose a ticket...</option>
                  <option :for={t <- @tickets} value={t.id}>{t.subject}</option>
                </select>
              </div>
              <button type="submit" class="w-full px-4 py-2 text-sm bg-purple-600 text-white rounded-lg hover:bg-purple-700">Run AI Triage</button>
            </form>
          </div>

          <div class="bg-white rounded-lg border border-gray-200 p-6">
            <h2 class="text-lg font-semibold text-gray-900 mb-4">SLA Check</h2>
            <button phx-click="check_sla" class="w-full px-4 py-2 text-sm bg-orange-600 text-white rounded-lg hover:bg-orange-700">
              Run SLA Check
            </button>
          </div>
          ...
          <div class="bg-white rounded-lg border border-gray-200 p-6">
            ...
            <div class="space-y-2">
              <div :for={integration <- [:front, :slack, :linear]} class="flex items-center justify-between">
                <span class="text-sm font-medium text-gray-700 capitalize">{integration}</span>
                <div class="flex gap-2">
                  <button phx-click="trip_breaker" phx-value-integration={integration} class="px-3 py-1 text-xs bg-red-100 text-red-700 rounded hover:bg-red-200">Trip</button>
                  <button phx-click="reset_breaker" phx-value-integration={integration} class="px-3 py-1 text-xs bg-green-100 text-green-700 rounded hover:bg-green-200">Reset</button>
                </div>
              </div>
            </div>
          </div>
