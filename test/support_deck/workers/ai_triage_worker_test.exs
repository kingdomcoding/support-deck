defmodule SupportDeck.Workers.AITriageWorkerTest do
  ...
  test "performs triage on a ticket" do
    ticket = create_ticket!(%{subject: "Database connection error", body: "Cannot connect to postgres"})

    assert :ok == SupportDeck.Workers.AITriageWorker.perform(
      %Oban.Job{args: %{"ticket_id" => ticket.id}}
    )
